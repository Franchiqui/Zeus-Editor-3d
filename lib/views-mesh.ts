    ...
    if (xr.length === 1 && zr.length === 1) {
      ...
      if (kx >= 1e-7 && kz >= 1e-7) {
        ...
        if (useSections) {
          ...
          baseContour = interpolateSection(sectionsSorted, yy, sectionsTargetN);
        } else {
          baseContour = top;
        }
 ...
 * menos de 3 puntos.
 */
export function polylineToPolygon(polyline: Polyline): Polygon | null {
  if (polyline.points.length < 3) return null;
 ...
 */
export function buildExtrudeMesh(views: Views, depth: number): Mesh {
...
export function buildExtrudeMeshes(
  ...
  for (let pi = 0; pi < polygons.length; pi++) {
    ...
    allFaces = allFaces.concat(
      mesh.faces.map((face) => face.map((idx) => idx + offset))
    );
  }
...
function earClipWithHoles(
  ...
  for (let i = 0; i < n; i++) combined.push(i);
  ...
  for (let hi = 0; hi < holeStarts.length; hi++) {
    ...
    for (let j = hs + 1; j < he; j++) {
      if (flat[j].x < flat[bestHoleIdx].x) bestHoleIdx = j;
    }
    ...
    const toInsert: number[] = [bestExtIdx];
    for (let j = bestHoleIdx; j < he; j++) toInsert.push(j);
    for (let j = hs; j < bestHoleIdx; j++) toInsert.push(j);
    toInsert.push(bestExtIdx);
    combined.splice(extPos + 1, 0, ...toInsert);
  }

  const points: Point2D[] = combined.map((i) => flat[i]);
