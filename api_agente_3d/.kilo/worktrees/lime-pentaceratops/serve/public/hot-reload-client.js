...
(function() {
  ...
  function getWebSocketUrl() {
    ...
    // 3. Si estamos en localhost, usar ws://localhost:3032
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
      return `ws://localhost:${port || 3032}`;
    }
  ...
  function connectWebSocket(projectId, fallbackTried = false) {
    ...
    try {
      ...
      ws.onerror = (error) => {
        ...
        // Si falla en entorno túnel/producción, intentar fallback a localhost:3032 una sola vez
        if (!fallbackTried && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
          console.warn('[HotReload] Intentando fallback a ws://localhost:3032 ...');
          setTimeout(() => {
            try {
      ...
      ws.onclose = () => {
        ...
        if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
          ...
          console.warn('[HotReload] Máximo de intentos alcanzado, intentando fallback a ws://localhost:3032 ...');
