...
const app = express();
// Base directory: when packaged with pkg, use the folder of the executable; otherwise use the source directory
const isPkg = typeof process.pkg !== 'undefined';
const baseDir = process.env.SERVE_BASE_DIR || (isPkg ? path.dirname(process.execPath) : __dirname);
// Directorios escribibles: en producción el paquete puede estar en Program Files (solo-lectura),
// así que main.js inyecta SERVE_LOGS_DIR / UPLOADS_DIR apuntando a %APPDATA%.
const LOGS_DIR = process.env.SERVE_LOGS_DIR || path.join(baseDir, 'logs');
const UPLOADS_DIR = process.env.UPLOADS_DIR || path.join(baseDir, 'uploads');
let previewPort;
...
function schedulePocketBaseSave(projectId, projectPath, userToken = null) {
  ...
  pocketBaseSaveTimeout = setTimeout(async () => {
    ...
    try {
      ...
      const defaultNextJsUrl = isProduction ? 'https://zeus-ia.com' : 'http://localhost:3001';
...
app.post('/api/save-local-file', async (req, res) => {
  ...
  try {
    ...
    if (!filePath || content === undefined || !projectRoot) {
      return res.status(400).json({ success: false, error: 'filePath, content, and projectRoot are required' });
    }
...
/**
 * POST /api/tutorial/screenshot
 * Captures a screenshot of the preview app running on the given port.
 * Returns { screenshot: "<base64 PNG>" }
 */
app.post('/api/tutorial/screenshot', async (req, res) => {
  const { previewPort: pPort = 3000, mousePosition } = req.body || {};

  try {
    // Strategy 1: Use puppeteer if available
    let puppeteer;
    ...
    if (puppeteer) {
      ...
      try {
        ...
        await page.goto(`http://127.0.0.1:${pPort}`, { waitUntil: 'networkidle2', timeout: 10000 });

        // Inject virtual mouse cursor if position provided
        if (mousePosition && typeof mousePosition.x === 'number' && typeof mousePosition.y === 'number') {
          await page.evaluate((pos) => {
            const existing = document.getElementById('zeus-virtual-cursor');
            if (existing) existing.remove();
            const cursor = document.createElement('div');
            cursor.id = 'zeus-virtual-cursor';
          ...
          // Small wait for paint
          await new Promise(r => setTimeout(r, 150));
        }

        const screenshotBuffer = await page.screenshot({ type: 'png', encoding: 'base64' });
        await browser.close();
        return res.json({ screenshot: 'data:image/png;base64,' + screenshotBuffer, method: 'puppeteer' });
      } catch (puppErr) {
        console.warn('[Tutorial/Screenshot] Puppeteer failed:', puppErr.message);
        if (browser) try { await browser.close(); } catch {}
      }
    }

    // Strategy 2: Use playwright if available
    let playwright;
    ...
    if (playwright) {
      ...
      try {
        ...
        await page.goto(`http://127.0.0.1:${pPort}`, { waitUntil: 'networkidle0', timeout: 10000 });
        const screenshotBuffer = await page.screenshot({ type: 'png' });
        await browser.close();
        return res.json({ screenshot: 'data:image/png;base64,' + screenshotBuffer.toString('base64'), method: 'playwright' });
      } catch (pwErr) {
        console.warn('[Tutorial/Screenshot] Playwright failed:', pwErr.message);
        if (browser) try { await browser.close(); } catch {}
      }
    }

    // Strategy 3: Minimal HTTP GET fallback — check if app is reachable
    try {
      const http = require('http');
      await new Promise((resolve, reject) => {
        const req2 = http.get(`http://127.0.0.1:${pPort}`, (r) => { r.resume(); resolve(r.statusCode); });
        req2.on('error', reject);
        req2.setTimeout(3000, () => { req2.destroy(); reject(new Error('timeout')); });
      });
      // App is reachable but we can't take a screenshot without a browser
      return res.json({ screenshot: null, method: 'none', message: `App is reachable on port ${pPort} but screenshot requires puppeteer or playwright` });
    } catch {
      return res.json({ screenshot: null, method: 'none', message: `App not reachable on port ${pPort}` });
    }

  } catch (error) {
    console.error('[Tutorial/Screenshot] Error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Screenshot failed', screenshot: null });
  }
});

/**
 * POST /api/tutorial/execute-action
 * Executes a user action in the preview app.
 * Supports: click, type, scroll, wait, navigate, highlight
 */
app.post('/api/tutorial/execute-action', async (req, res) => {
  const { type: actionType, target, value, previewPort: pPort = 3000 } = req.body || {};
  ...
  try {
    ...
    if (puppeteer) {
      ...
      try {
        ...
        const page = await browser.newPage();
        await page.goto(`http://127.0.0.1:${pPort}`, { waitUntil: 'domcontentloaded', timeout: 8000 });
        ...
        switch (actionType?.toLowerCase()) {
          ...
          case 'scroll': {
            const scrollY = parseInt(value || '300', 10);
            await page.evaluate((y) => window.scrollBy(0, y), scrollY);
            break;
          }
          case 'navigate': {
            if (value) {
              const url = value.startsWith('http') ? value : `http://127.0.0.1:${pPort}${value}`;
          ...
          default:
            // No-op or wait
            await new Promise(r => setTimeout(r, Math.min(parseInt(value || '500', 10), 3000)));
        }

        await browser.close();
        return res.json({ success: true, actionType, method: 'puppeteer' });
      } catch (err) {
        console.warn('[Tutorial/ExecuteAction] Puppeteer failed:', err.message);
        if (browser) try { await browser.close(); } catch {}
      }
    }
    ...
    if (playwright) {
      ...
      try {
        ...
        switch (actionType?.toLowerCase()) {
          ...
          case 'scroll': {
            await page.evaluate((y) => window.scrollBy(0, y), parseInt(value || '300', 10));
            break;
          }
          case 'navigate': {
            if (value) await page.goto(value.startsWith('http') ? value : `http://127.0.0.1:${pPort}${value}`);
            break;
          }
        }

        await browser.close();
        return res.json({ success: true, actionType, method: 'playwright' });
      } catch (err) {
        console.warn('[Tutorial/ExecuteAction] Playwright failed:', err.message);
        if (browser) try { await browser.close(); } catch {}
      }
    }

    // Fallback: log the action (no browser available)
    console.log(`[Tutorial/ExecuteAction] No browser available. Simulating: ${actionType} on ${target || 'page'} with value: ${value}`);
    if (actionType === 'wait') {
      await new Promise(r => setTimeout(r, Math.min(parseInt(value || '500', 10), 3000)));
    }
    return res.json({ success: true, actionType, method: 'simulated', message: 'Action simulated (no browser available)' });

  } catch (error) {
    console.error('[Tutorial/ExecuteAction] Error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Action execution failed' });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// Desktop Automation Endpoints (Real mouse, keyboard, screen recording)
// ─────────────────────────────────────────────────────────────────────────────

const desktopAutomationPath = require.resolve('./desktop-automation');

/**
 * Helper to get a fresh copy of the desktop automation module.
 * Allows code changes to take effect without restarting the serve process.
 */
function getDesktopAuto() {
  delete require.cache[desktopAutomationPath];
  return require(desktopAutomationPath);
}

const desktopAuto = require('./desktop-automation');

/**
 * POST /api/desktop/action
 * Execute a real desktop action (click, type, scroll, keypress, wait).
 * Body: { type, target?, value?, x?, y? }
 */
app.post('/api/desktop/action', async (req, res) => {
  try {
    const { type, target, value, x, y } = req.body || {};
    if (!type) {
      return res.status(400).json({ error: 'type is required' });
    }
    const result = await desktopAuto.executeDesktopAction({ type, target, value, x, y });
    return res.json({ success: true, actionType: type, method: 'desktop', realCursorPosition: result || null });
  } catch (error) {
    console.error('[Desktop/Action] Error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Desktop action failed' });
  }
});

/**
 * POST /api/desktop/screenshot
 * Capture a real desktop screenshot.
 * Returns { screenshot: "data:image/png;base64,...", path }
 */
app.post('/api/desktop/screenshot', async (req, res) => {
  try {
    const { filename = 'desktop-screenshot.png', clip = null, cursorOffset = null, cursorPosition = null, cursorImagePath = null } = req.body || {};
    const freshDesktopAuto = getDesktopAuto();
    const tempDir = path.join(LOGS_DIR, 'screenshots');
    const outputPath = path.join(tempDir, filename);
    await freshDesktopAuto.screenshot(outputPath, clip, cursorOffset, cursorPosition, cursorImagePath);
    const base64 = freshDesktopAuto.screenshotToBase64(outputPath);
    return res.json({
      screenshot: base64,
      path: outputPath,
      x: clip?.x ?? 0,
    ...
    console.error('[Desktop/Screenshot] Error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Screenshot failed', screenshot: null });
  }
});

/**
 * POST /api/desktop/recording/start
 * Start desktop screen recording with FFmpeg.
 * Body: { outputPath? } — defaults to logs/recordings/tutorial-{timestamp}.mp4
 */
app.post('/api/desktop/recording/start', async (req, res) => {
  try {
    const { outputPath, clip, quality } = req.body || {};
    const recordingsDir = path.join(LOGS_DIR, 'recordings');
    const finalPath = outputPath || path.join(recordingsDir, `tutorial-${Date.now()}.mp4`);

    if (desktopAuto.isRecording()) {
      return res.status(409).json({ error: 'A recording is already in progress' });
    }

    const result = await desktopAuto.startRecording(finalPath, clip, quality);
    return res.json({ success: true, outputPath: result.outputPath, pid: result.pid });
  } catch (error) {
    console.error('[Desktop/Recording] Start error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Failed to start recording' });
  }
});

/**
 * POST /api/desktop/recording/stop
 * Stop desktop screen recording.
 * Returns { outputPath, message }
 */
app.post('/api/desktop/recording/stop', async (req, res) => {
  try {
    const result = await desktopAuto.stopRecording();
    return res.json({ success: true, ...result });
  } catch (error) {
    console.error('[Desktop/Recording] Stop error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Failed to stop recording' });
  }
});

/**
 * GET /api/desktop/recording/file?path=<absolutePath>
 * Sirve el MP4 grabado por /recording/start|stop para que el navegador lo
 * reproduzca (<video>) o lo descargue como Blob y lo suba como vídeo base.
 * Validado dentro de logs/recordings para evitar path traversal.
 */
app.get('/api/desktop/recording/file', async (req, res) => {
  try {
    const requested = req.query.path;
    if (typeof requested !== 'string' || !requested) {
      return res.status(400).json({ error: 'path is required' });
    }
    const recordingsDir = path.resolve(path.join(LOGS_DIR, 'recordings'));
    const resolved = path.resolve(requested);

    // Rechazar rutas relativas con .. y exigir que el fichero quede dentro del
    // directorio de grabaciones (comparación robusta con separador final).
    const isInside =
      resolved === recordingsDir || resolved.startsWith(recordingsDir + path.sep);
    if (!isInside) {
      return res.status(403).json({ error: 'Path outside recordings directory' });
    }
    ...
    console.error('[Desktop/Recording/File] error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Failed to serve recording file' });
  }
});
...
// Endpoint para reiniciar PocketBase
app.post('/api/restart-pocketbase', async (req, res) => {
  try {
    console.log('[RESTART-POCKETBASE] Restarting PocketBase on port 8236');

    const { exec } = require('child_process');

    // Start PocketBase on port 8236
    const pocketbasePath = path.join(baseDir, '..', 'pocketbase');
    const command = process.platform === 'win32'
      ? `cd ${pocketbasePath} && pocketbase serve --http 0.0.0.0:8236`
      : `cd ${pocketbasePath} && ./pocketbase serve --http 0.0.0.0:8236`;

    exec(command, (error, stdout, stderr) => {
      if (error) {
        console.error('[RESTART-POCKETBASE] Error:', error);
        return res.status(500).json({ success: false, error: error.message });
      }
      console.log('[RESTART-POCKETBASE] PocketBase restarted successfully');
...
// Endpoint para matar proceso en un puerto específico
app.post('/api/kill-port', async (req, res) => {
  try {
    ...
    exec(command, (error, stdout, stderr) => {
      ...
      console.log('[KILL-PORT] Process killed on port:', port);
...
// Endpoint para iniciar el servidor de desarrollo (npm run dev)
app.post('/api/start-dev', async (req, res) => {
  try {
    const { projectPath, port, services } = req.body || {};
    ...
    console.log('[START-DEV] Starting development server for:', projectPath, 'on port:', devPort);
    console.log('[START-DEV] Services to start:', servicesList.filter(s => s && s.enabled && s.command).length);
    ...
    // Check if project has its own pocket-base folder
    const hasPocketBase = fs.existsSync(path.join(resolvedPath, 'pocket-base'));
    console.log('[START-DEV] Project has pocket-base folder:', hasPocketBase);

    // If project has pocket-base, update .env and .env.local to use the correct port
    if (hasPocketBase) {
      const pocketbaseUrl = 'http://127.0.0.1:8236';
      ...
      const updateEnvFile = (filePath) => {
        try {
          ...
          // Update or add NEXT_PUBLIC_POCKETBASE_URL
          const nextPublicRegex = /^NEXT_PUBLIC_POCKETBASE_URL=.*/m;
          if (nextPublicRegex.test(content)) {
            content = content.replace(nextPublicRegex, `NEXT_PUBLIC_POCKETBASE_URL=${pocketbaseUrl}`);
          } else {
            content += `\nNEXT_PUBLIC_POCKETBASE_URL=${pocketbaseUrl}`;
          }

          // Update or add POCKETBASE_URL
          const pocketbaseRegex = /^POCKETBASE_URL=.*/m;
          if (pocketbaseRegex.test(content)) {
            content = content.replace(pocketbaseRegex, `POCKETBASE_URL=${pocketbaseUrl}`);
          } else {
            content += `\nPOCKETBASE_URL=${pocketbaseUrl}`;
          }

          fs.writeFileSync(filePath, content.trim() + '\n');
          console.log(`[START-DEV] Updated ${path.basename(filePath)} with PocketBase URL: ${pocketbaseUrl}`);
        } catch (error) {
          console.warn(`[START-DEV] Could not update ${path.basename(filePath)}:`, error.message);
        }
      };

      // Update .env.local
      updateEnvFile(path.join(resolvedPath, '.env.local'));

      // Update .env
      updateEnvFile(path.join(resolvedPath, '.env'));
    }

    // Set currentProject with the provided path
    currentProject = {
    ...
    try {
      const serverInfo = await startDevServer(resolvedPath, 'local-project', devPort);
      currentProject.status = 'ready';
      currentProject.serverInfo = serverInfo;

      console.log('[START-DEV] Development server started successfully at:', serverInfo.url);

      // Start the app's auxiliary services (generic, user-defined list).
      if (servicesList.length > 0) {
        console.log('[START-DEV] Starting app services...');
        try {
          await startServices(resolvedPath, servicesList);
          console.log('[START-DEV] App services started');
        } catch (error) {
          console.error('[START-DEV] Error starting app services:', error);
          // Don't fail the whole request if a service fails to start
        }
      }

      res.json({ success: true, url: serverInfo.url, hasPocketBase, port: devPort });
    } catch (error) {
      console.error('[START-DEV] Error starting development server:', error);
...
// Función para iniciar los servicios auxiliares de la app objetivo (lista
// genérica, editable desde el editor de Tutoriales). Cada servicio indica su
// puerto y el comando a ejecutar en la carpeta del proyecto. El token {port}
// en el comando se sustituye por el puerto, y además se inyecta env.PORT.
//
// Se trackean los procesos lanzados (y sus puertos) para poder pararlos al
// cerrar el preview (POST /api/stop-services → stopStartedServices).
let startedServiceChildren = [];
let startedServicePorts = [];

async function stopStartedServices() {
  const { exec } = require('child_process');
  // Matar el árbol de cada proceso lanzado (cmd.exe + sus hijos en Windows).
  for (const child of startedServiceChildren) {
    try {
      if (child && child.pid) {
        if (process.platform === 'win32') {
          exec(`taskkill /PID ${child.pid} /T /F`, () => { /* ignora errores */ });
        } else {
          try { process.kill(child.pid, 'SIGKILL'); } catch (e) { /* ya muerto */ }
        }
      }
    } catch (e) { /* no crítico */ }
  }
  // Por si el árbol no cubre todo, liberar también los puertos.
  for (const port of startedServicePorts) {
    if (port > 0) {
      try { await freePort(port); } catch (e) { /* no crítico */ }
    }
  }
  startedServiceChildren = [];
  startedServicePorts = [];
  console.log('[SERVICES] Stopped previously started services');
}

async function startServices(projectPath, services) {
  const { spawn } = require('child_process');
  const list = Array.isArray(services) ? services : [];
  console.log('[SERVICES] Starting app services for:', projectPath, `(${list.length} configured)`);

  // Parar los servicios de un arranque anterior (si no se pararon al cerrar).
  await stopStartedServices();

  for (const s of list) {
    if (!s || !s.enabled || !s.command) continue;
    const port = Number(s.port) || 0;
    try {
      // Liberar el puerto si está ocupado (evita "port in use").
      if (port > 0) {
        try { await freePort(port); await sleep(500); } catch (e) { /* no crítico */ }
      }
      const cmd = String(s.command).split('{port}').join(String(port || ''));
      const env = { ...process.env, ...(port > 0 ? { PORT: String(port) } : {}) };
      console.log(`[SERVICES] Starting "${s.name || 'service'}" (port ${port || '-'}) -> ${cmd}`);
      ...
      child.on('error', (err) => console.error(`[SERVICES] Error starting "${s.name}":`, err.message));
      if (child.stderr) {
        child.stderr.on('data', (d) => {
          const msg = d.toString().trim();
          if (msg) console.warn(`[SERVICES] "${s.name}" stderr: ${msg}`);
        });
      }
      // Trackear para poder pararlos al cerrar el preview.
      startedServiceChildren.push(child);
      if (port > 0) startedServicePorts.push(port);
    } catch (error) {
      console.error(`[SERVICES] Could not start "${s.name}":`, error.message);
      // No abortar el resto de servicios.
    }
  }

  console.log('[SERVICES] Done starting app services');
}

// Parar los servicios auxiliares arrancados por startServices (al cerrar preview).
...
// Endpoint para establecer la ruta del proyecto directamente (sin ZIP)
app.post('/api/set-project-path', async (req, res) => {
  try {
    ...
    console.log('[SET-PROJECT-PATH] Setting project path:', projectPath);
    ...
    currentProject = {
      ...
      path: resolvedPath,
      status: 'ready',
      serverInfo: { url: `http://localhost:${previewPort}` }
    };
    
    console.log('[SET-PROJECT-PATH] Project path set successfully:', resolvedPath);
...
function resolveAutoBuildFixTarget(req, override) {
  ...
  return isProductionEnv ? 'https://zeus-ia.com' : 'http://localhost:3001';
}
...
const upload = multer({
  storage,
  limits: {
    fileSize: 8192 * 1024 * 1024 // 8GB lÃ­mite
  },
...
const cursorStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const cursorsDir = path.join(__dirname, 'cursors');
    if (!fs.existsSync(cursorsDir)) fs.mkdirSync(cursorsDir, { recursive: true });
    cb(null, cursorsDir);
  },
  filename: (req, file, cb) => {
    cb(null, `${Date.now()}-${file.originalname}`);
...
const uploadCursor = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const allowed = ['.cur', '.ico', '.png', '.svg', '.bmp', '.gif', '.jpg', '.jpeg'];
    console.log('[Cursor Upload] Checking file:', file.originalname, file.mimetype);
    if (allowed.includes(ext)) {
      cb(null, true);
    } else {
      cb(new Error('Solo se permiten cursores .cur, .ico, .png, .svg, .bmp, .gif, .jpg'), false);
    }
  }
});
...
fs.ensureDirSync(path.join(__dirname, 'cursors'));

/**
 * POST /api/desktop/cursor
 * Upload a custom cursor image. Returns the local path to use in screenshot/recording.
 * Accepts .cur, .ico, .png, .svg, .bmp, .gif, .jpg
 */
app.post('/api/desktop/cursor', uploadCursor.single('cursor'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No cursor file uploaded' });
    }
    return res.json({
      path: req.file.path,
    ...
    console.error('[Desktop/Cursor] Error:', error?.message || error);
    return res.status(500).json({ error: error?.message || 'Cursor upload failed' });
  }
});

// Crear directorios necesarios (continuación)
...
const projectsDir = endsWithCurrentProject ? path.dirname(envProjectsDir) : (envProjectsDir || path.join(baseDir, 'projects'));
...
async function cleanupCurrentProject(options = {}) {
  ...
  try {
    if (currentProject) {
      ...
      // Extra step for PocketBase projects: ensure port 7080 is free
      try {
        const pocketBasePath = path.join(pathToClean, 'pocket-base');
        if (fs.existsSync(pocketBasePath)) {
            console.log('[Cleanup] PocketBase project detected. Ensuring port 7080 is free...');
            await freePort(7080);
        }
      } catch (e) {
      ...
      try {
        ...
        if (fs.existsSync(pbDatosPath)) {
            console.log('[Cleanup] PB_Datos folder detected. Ensuring port 3002 is free...');
            await freePort(3002);
        }
      } catch (e) {
          console.warn(`[Cleanup] Could not ensure port 3002 is free: ${e?.message || e}`);
      }
      ...
      if (!skipDirectoryRemoval) {
        try {
          ...
          try {
            ...
            // Last resort: try robust removal
            console.log('[Cleanup] Trying robust removal as last resort...');
            try {
              await robustRemove(pathToClean);
      ...
      try {
        ...
        if (fs.existsSync(pathToClean)) {
          ...
          try {
            ...
            await robustRemove(pathToClean, 20, 1000);
...
async function robustRemove(dirPath) { // Removed retries and delay parameters
  console.log(`[RobustRemove] Starting single attempt removal of ${dirPath}`);
  ...
  try {
    if (!fs.existsSync(dirPath)) {
      console.log(`[RobustRemove] Directory does not exist: ${dirPath}`);
      return;
    }
    
    fs.removeSync(dirPath);
    console.log(`[RobustRemove] Directory ${dirPath} removed successfully.`);
    return;
  } catch (err) {
    console.error(`[RobustRemove] Error removing ${dirPath}:`, err);
    throw err;
  }
}
        ...
        if (isDirectory) {
          try {
            ...
            if (dirErr.code !== 'EEXIST') {
              console.warn(`[ExtractZip] Warning creating directory ${entryName}:`, dirErr.message);
            }
          }
        } else {
          try {
            ...
            if (stats.isDirectory()) {
              console.warn(`[ExtractZip] Skipping ${entryName}: path is already a directory`);
              continue;
            }
          } catch (statErr) {
          ...
          try {
            ...
            if (extractedCount <= 10 || extractedCount % 100 === 0) {
              console.log(`[ExtractZip] Extracted file ${extractedCount}: ${entryName}`);
            }
          } catch (extractErr) {
            console.error(`[ExtractZip] Error extracting ${entryName}:`, extractErr.message);
            logZipError(zipPath, extractPath, extractErr, entryName);
          }
        }
      }
...
function runNpmCommand(projectPath, command) {
  return new Promise((resolve, reject) => {
    ...
    let lastOutputChunk = '';
    const skipLoopDetection = /\bcap\s+(add|sync)\s+android\b/i.test(command) || /install|setup-pocketbase|postinstall/i.test(command);
    const MAX_REPEATED_OUTPUT = 30; // Maximum number of repeated outputs before considering it a loop
    ...
    npmProcess.stdout.on('data', (data) => {
      ...
      // Detect repeated output (possible infinite loop)
      if (!skipLoopDetection) {
        const now = Date.now();
        const normalizedChunk = dataStr.trim().replace(/\s+/g, ' ');
        // Only count if the output chunk is actually repeated and arrives within 5 seconds
        if (now - lastOutputTime < 5000 && normalizedChunk && normalizedChunk === lastOutputChunk) {
          repeatedOutputCount++;
          if (repeatedOutputCount >= MAX_REPEATED_OUTPUT) {
            console.error(`[runNpmCommand] ⚠️ Detected possible infinite loop (${repeatedOutputCount} repeated outputs). Killing process...`);
        ...
        lastOutputTime = now;
        lastOutputChunk = normalizedChunk;
      }
    });
...
function startDevServer(projectPath, projectId, previewServerPort) {
  return (async () => {
    // Usar el puerto especificado o 3000 por defecto
    const port = previewServerPort || 3000;
    ...
    // Determinar el puerto para la aplicación. Usar el puerto especificado o 3000 por defecto.
    let portNum = port;
    ...
    try {
      if (fs.existsSync(pocketBasePath) && fs.lstatSync(pocketBasePath).isDirectory()) {
        console.log(`[INFO] "pocket-base" folder found. Using port ${portNum}.`);
      } else {
        // Si no hay pocket-base, pero es un proyecto Next.js, usar el puerto especificado.
        console.log(`[INFO] "pocket-base" folder not found. Attempting to use port ${portNum} for Next.js app.`);
      }
    } catch (e) {
      console.log(`[INFO] Error checking pocket-base folder. Attempting to use port ${portNum} for Next.js app.`);
    }
...
function isMobileApp(projectPath) {
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      const hasCapacitorCore = packageJson.dependencies && packageJson.dependencies['@capacitor/core'] !== undefined;
      const hasCapacitorCli = packageJson.dependencies && packageJson.dependencies['@capacitor/cli'] !== undefined;
      const hasCapacitorAndroid = packageJson.dependencies && packageJson.dependencies['@capacitor/android'] !== undefined;
...
app.post('/api/run-electron-command', async (req, res) => {
  ...
  try {
    ...
    if (isDev) {
      ...
      const shellCmd = 'npx wait-on http://localhost:3000 && npx electron .';
...
app.post('/api/build-apk', async (req, res) => {
  ...
  try {
    ...
    // Build the Android APK using Gradle directly
    console.log('[BUILD] Building Android APK with Gradle...');
    const gradleProcess = spawn('.\\gradlew', ['assembleDebug'], {
...
app.get('/api/download-apk', (req, res) => {
  ...
  try {
    ...
    // Try to find APK in alternative locations if default doesn't exist
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      ...
      if (port === 3032) {
        ...
        console.error(`[ERROR] Both ports (3032 and 3033) are in use. Could not start preview server.`);
        process.exit(1);
      }
    } else {
...
app.post('/api/trigger-upload-and-deploy', async (req, res) => {
    ...
    try {
        ...
        const restartResponse = await internalFetch({
            hostname: 'localhost',
            port: process.env.PORT || 3032, // Use the server's own port
  ...
  try {
    ...
    const processTunnelUrl = (text) => {
      ...
      if (urlMatch && !tunnelUrl) {
        ...
        const tunnelUrlPath = path.join(baseDir, 'tunnel-url.txt');
...
function startAdditionalServices() {
  ...
  if (fs.existsSync(restartApiBatPath)) {
    ...
    try {
      ...
      // Sin detached: en Node/Windows, detached:true + cmd /c puede hacer que
      // cmd cree su propia consola visible aunque windowsHide esté a true.
...
(async () => {
  const PREVIEW_SERVER_PORT = 3032;
  if (await isPortInUse(PREVIEW_SERVER_PORT)) {
    ...
    server.on('listening', async () => {
      ...
      // Túnel Cloudflare DESACTIVADO por defecto: la app trabaja en local.
      // Para reactivarlo, arrancar el servidor con ENABLE_TUNNEL=1.
      if (process.env.ENABLE_TUNNEL === '1') {
        ...
        console.log('[TUNNEL] Desactivado (modo local). Establecer ENABLE_TUNNEL=1 para reactivarlo.');
      }
...
app.post('/api/refresh-project-from-pocketbase', async (req, res) => {
  ...
  console.log('[RefreshProject] isPkg (packaged):', isPkg);
