/* Inspecciona el contenido del modal catálogo Objeto 3D. */
...
(async () => {
  ...
  const info = await page.evaluate(() => {
    ...
    return [...d.querySelectorAll('button, [role="button"], a, [class*="cursor-pointer"]')]
      .slice(0, 25)
      .map((e) => `${e.tagName}:${(e.textContent || '').trim().slice(0, 70)}`);
  });
