...
export default function MainNavbar({
  ...
  const isIconLibraryOpen = iconLibraryContext?.isModalOpen ?? false;
  const toggleIconLibrary = iconLibraryContext?.toggleModal ?? (() => {});
  ...
  return (
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="bg-gray-900 border-gray-800 text-white min-w-[150px]">
                <DropdownMenuItem onClick={() => router.push('/edit-html')} className="hover:bg-green-600 cursor-pointer flex gap-2"><Code2 className="w-4 h-4 text-emerald-400" /> {t('nav.editorHtml')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-video')} className="hover:bg-green-600 cursor-pointer flex gap-2"><Video className="w-4 h-4 text-red-400" /> {t('nav.editorVideo')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-audio')} className="hover:bg-green-600 cursor-pointer flex gap-2"><Music className="w-4 h-4 text-green-400" /> {t('nav.editorAudio')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-imagen')} className="hover:bg-green-600 cursor-pointer flex gap-2"><ImageIcon className="w-4 h-4 text-blue-400" /> {t('nav.editorImagen')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-gif')} className="hover:bg-green-600 cursor-pointer flex gap-2"><ImageIcon className="w-4 h-4 text-purple-400" /> {t('nav.editorGif')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-texto')} className="hover:bg-green-600 cursor-pointer flex gap-2"><File className="w-4 h-4 text-yellow-400" /> {t('nav.editorDocumento')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-tutorials')} className="hover:bg-green-600 cursor-pointer flex gap-2"><GraduationCap className="w-4 h-4 text-amber-400" /> {t('nav.editorTutorials')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-fuente')} className="hover:bg-green-600 cursor-pointer flex gap-2"><Type className="w-4 h-4 text-pink-400" /> {t('nav.editorFuentes')}</DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/edit-icon')} className="hover:bg-green-600 cursor-pointer flex gap-2"><ImageIcon className="w-4 h-4 text-orange-400" /> {t('nav.editorIconos')}</DropdownMenuItem>
    ...
    <ModelConfigModal isOpen={isModelConfigOpen} onClose={() => setIsModelConfigOpen(false)} onSaved={fetchModels} />
    <IconLibraryModal open={isIconLibraryOpen} onOpenChange={toggleIconLibrary} />
    </>
  );
}
