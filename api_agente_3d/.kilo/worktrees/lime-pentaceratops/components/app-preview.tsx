...
export default function AppPreview({ zipUrl, appName, isOpen, onClose }: AppPreviewProps) {
  ...
  const extractAndRender = useCallback(async () => {
    ...
    try {
      ...
      if (contentType.includes('text/html')) {
        throw new Error(t('app.htmlInsteadZip'));
      }
      if (contentType.includes('application/json')) {
        throw new Error(t('app.jsonInsteadZip'));
      }

      const blob = await response.blob();
      if (blob.size === 0) {
        throw new Error(t('app.emptyZip'));
      }
      if (blob.size < 4) {
        throw new Error(t('app.smallZip'));
      }
      ...
      if (!htmlFile) {
        throw new Error(t('app.noHtmlInZip') + files.join(', '));
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-6xl h-[90vh] mx-4 bg-gray-900 rounded-xl shadow-2xl border border-gray-700 flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 relative overflow-hidden bg-gray-950">
          {loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900">
              ...
              <p className="text-gray-400 text-sm">{t('app.extractingApp')}</p>
            </div>
          )}
