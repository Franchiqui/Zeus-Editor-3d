 ...
 */
function normalizeInBox(poly, bb) {
    return poly.map((q) => {
        const out = {
            x: (q.x - bb.minX) / bb.w,
            y: (q.y - bb.minY) / bb.h,
        ...
        if (q.hIn)
            out.hIn = { x: (q.hIn.x - bb.minX) / bb.w, y: (q.hIn.y - bb.minY) / bb.h };
        if (q.hOut)
            out.hOut = { x: (q.hOut.x - bb.minX) / bb.w, y: (q.hOut.y - bb.minY) / bb.h };
        return out;
    });
}
function extrudeSinglePolygon(polygon, depth, holes = []) {
    ...
    const front = (0, geometry_1.flattenPolygon)(normalizeInBox(polygon, mainBox), curveSteps);
    if (front.length < 3)
        return { vertices: [], faces: [] };
    ...
    for (const hole of holes) {
        ...
        if (poly.length < 3)
            continue;
        const flat = (0, geometry_1.flattenPolygon)(normalizeInBox(poly, mainBox), curveSteps);
        if (flat.length < 3)
            continue;
        ...
        if (shoelace(flatClosed) > 0)
            flatClosed.reverse();
        normHoles.push({ flat: flatClosed, depth: (0, geometry_1.holeDepth)(hole) });
    }
    const isBlind = (hd) => hd > 0 && hd < depth;
    ...
    const throughBack = [];
    const blindFront = [];
    const blindFloor = []; // anilla del suelo en su orden de tri.
    const blindFloorRing = []; // puntos del suelo (shoelace positivo)
    const blindWallBack = []; // anilla del suelo en orden de pared
    ...
    const allHoleFronts = [...throughFront, ...blindFront];
    // --- Triangulación de una tapa (con agujeros) usando ShapeUtils ---
    const triangulate = (contour, hs) => {
        try {
            const raw = three_1.ShapeUtils.triangulateShape(contour.map((q) => new three_1.Vector2(q.x, q.y)), hs.map((h) => h.map((q) => new three_1.Vector2(q.x, q.y))));
            const t = (Array.isArray(raw) ? raw : []);
            if (t.length)
                return t;
        }
        catch {
        ...
        for (let i = 0; i + 2 < ear.length; i += 3)
            t.push([ear[i], ear[i + 1], ear[i + 2]]);
        return t;
    };
    const makeOffsets = (flat) => {
        const offs = [];
        let acc = 0;
        for (const f of flat) {
            offs.push(acc);
    ...
    const idAt = (flat, offs, i) => {
        for (let g = flat.length - 1; g >= 0; g--) {
            if (i >= offs[g])
                return flat[g][i - offs[g]];
        }
        return flat[0][0];
    };
    ...
    const flat3dFront = [frontCap, ...allHoleFronts];
    ...
    const flat3dBack = [backCap, ...throughBack];
